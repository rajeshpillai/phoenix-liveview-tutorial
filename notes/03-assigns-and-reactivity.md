...
### 3. The `_target` Field in `phx-change` Events

When a `phx-change` event fires inside a form, the params include a special `_target`
key — a list indicating which input triggered the change. This lets you handle each
field independently:

```elixir
# The form has three inputs: name, color, count
# Typing in the "name" field sends: %{"_target" => ["name"], "name" => "Alice", ...}
...
def handle_event("update_reactive", %{"_target" => ["count"], "count" => count}, socket) do
  {:noreply, assign(socket, count: String.to_integer(count), count_rendered_at: now_str())}
end
```

Pattern-matching on `_target` avoids unnecessary work — if the user changed only the
color picker, you only update the color assign and its timestamp.

> **`phx-debounce` on text inputs:** Add `phx-debounce="200"` to text inputs inside
> `phx-change` forms to avoid flooding the server with events on every keystroke:
> ```heex
> <input type="text" name="name" phx-debounce="200" class="input" />
> ```
> Without debounce, each character fires a separate event. With `phx-debounce="200"`,
> the event fires 200ms after the user stops typing.

---

### 4. Forms Without Ecto
...
**Converting a map of errors to keyword list format:** If your validation returns a
plain map (e.g., `%{username: "too short", email: "invalid"}`), convert it before
passing to `to_form`:
...
# Usage:
to_form(params, as: "profile", errors: build_form_errors(errors))
```

---

### 5. Forms With Ecto Changesets
