...
#   OSS_REMOTE=https://github.com/algorisys-oss/elixir-katas.git ./scripts/publish-oss.sh --push
#
...
OSS_REMOTE="${OSS_REMOTE:-https://github.com/algorisys-oss/phx-liveview-tutorial.git}"
...
if [[ "$DRY_RUN" == false && -z "$OSS_REMOTE" ]]; then
    error "OSS_REMOTE is not set. Export it or pass it inline:\n  OSS_REMOTE=https://github.com/algorisys-oss/phx-liveview-tutorial.git $0 --push"
fi
...
(cd "$WORK_DIR" && find . -maxdepth 3 -not -path '*/deps/*' -not -path '*/_build/*' -not -path './.git/*' | sort)

echo ""
info "mix.exs project info:"
grep -E '(app:|version:)' "$WORK_DIR/mix.exs" || true
